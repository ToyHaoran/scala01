...
object IODemo extends App{
    ...
    val source = Source.fromFile("G:/person.txt","UTF-8")//绝对路径
    val source3 = Source.fromString("hello world")
    val source4 = Source.fromBytes("hello world".getBytes)//同上
    val source5 = Source.stdin    //从标准输入读取，CTRL+D结束     //命令行好像没法操作。
