...
object Setter {
  ...
  def makeSetter(
    ...
    case IntegerType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setInt(pos, row.getInt(offset))
    case LongType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setLong(pos, row.getLong(offset))
    case DoubleType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setDouble(pos, row.getDouble(offset))
    case FloatType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setFloat(pos, row.getFloat(offset))
    case ShortType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setInt(pos, row.getShort(offset))
    case ByteType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setInt(pos, row.getByte(offset))
    case BooleanType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setBoolean(pos, row.getBoolean(offset))
    case StringType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setString(pos, row.getString(offset))
    case BinaryType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setBytes(pos, row.getAs[Array[Byte]](offset))
    case TimestampType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setTimestamp(pos, row.getAs[java.sql.Timestamp](offset))
    case DateType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setDate(pos, row.getAs[java.sql.Date](offset))
    case t: DecimalType =>
      (stmt: PreparedStatement, row: Row, pos: Int, offset: Int) =>
        stmt.setBigDecimal(pos, row.getDecimal(offset))
    case _ =>
  ...
  def nullType(dataType: DataType): Int = {
    dataType match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"Can't translate type ${dataType.typeName} to database")
    }
  }
