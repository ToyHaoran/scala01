...
object DataFrameDemo {
    ...
    if(0){
        ...
        val df4_1 = spark.read.parquet("E:/hivedata1/*") //使用parquet文件创建
    ...
    if(0){
        //——————————————————————————RDD、DF、DS的相互转化————————————————————————————————————————————————
        //RDD转化为DF，参考创建DF，一种是直接map到Person，然后toDF；一种是case Class，然后createDF
    ...
    if(0){
        ...
        val hello = spark.read.textFile("F:\\桌面\\API\\Scala\\SparkDemo1\\src\\source\\hello.txt").as[String]
    ...
    if(0){
        ...
        //返回所有列名
        df.columns//以数组形式返回所有列名   res83: Array[String] = Array(key1, key2, key3)
    ...
    if(0){
        ...
        df.groupByKey(row=>row.getAs[String]("key1")).mapGroups((key1,group)=>{
            ...
            val key = key1
            var sum = 0
            for(i<-group){//group是Iterator[Row],比如key=bbb的为[bbb,3,4][bbb,4,6]
                //对Key2进行求和并且*2运算
                sum = sum + i.getInt(1)
            }
            res = res+(sum*2)
