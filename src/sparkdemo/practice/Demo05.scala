...
object Demo05 extends App{
    ...
    //读取文件，返回DF
    val  dfa = spark.read.json("F:\\桌面\\API\\Scala\\SparkDemo1\\src\\source\\practice\\exam3_table_A.json")
    val  dfb = spark.read.json("F:\\桌面\\API\\Scala\\SparkDemo1\\src\\source\\practice\\exam3_table_B.json")
    ...
    class Interval extends UserDefinedAggregateFunction{
        ...
        // 缓冲区中进行聚合时，所处理的数据的类型
        def bufferSchema: StructType = {StructType(StructField("sum", StringType) :: StructField("count", LongType) :: Nil)}
        // 返回值的数据类型，只能返回一列
        //    def dataType: DataType = StringType
        def dataType: StructType = {StructType(StructField("interval", StringType) :: StructField("count", LongType) :: Nil)}
